...
void CodedMaster::run()
{
	...
	//Test
	printf("CodedMaster.cc: For K = %d, the following factorization was selected: k = %d, q = %d\n", conf.getNumReducer(), conf.get_k(), conf.get_q());
	// printf("CodedMaster.cc: Slaves should start working now\n");
	...
	cout << rank << ": CODEGEN | Avg = " << setw(10) << avgTime/numWorker << "   Max = " << setw(10) << maxTime << endl;
	...
	cout << rank << ": MAP     | Avg = " << setw(10) << avgTime/numWorker << "   Max = " << setw(10) << maxTime << endl;

	//compute map memory time
	MPI::COMM_WORLD.Gather( &rTime, 1, MPI::DOUBLE, rcvTime, 1, MPI::DOUBLE, 0 );
	avgTime = 0;
	maxTime = 0;
	for( int i = 1; i <= numWorker; i++ ) {
	...
	cout << rank
	<< ": MAP MA | Avg = " << setw(10) << avgTime/numWorker
	...
	cout << rank << ": ENCODE  | Avg = " << setw(10) << avgTime/numWorker << "   Max = " << setw(10) << maxTime << endl;  

	//compute encode memory time
	MPI::COMM_WORLD.Gather( &rTime, 1, MPI::DOUBLE, rcvTime, 1, MPI::DOUBLE, 0 );
	avgTime = 0;
	maxTime = 0;
	for( int i = 1; i <= numWorker; i++ ) {
	...
	cout << rank
	<< ": ENCODE MA | Avg = " << setw(10) << avgTime/numWorker
	<< "   Max = " << setw(10) << maxTime << endl;
	...
	cout << rank << ": SHUFFLE | Sum = " << setw(10) << avgTime << "   Rate = " << setw(10) << avgRate/numWorker << " Mbps" << endl;
	...
	cout << rank << ": DECODE  | Avg = " << setw(10) << avgTime/numWorker << "   Max = " << setw(10) << maxTime << endl;  

	//compute decode memory time
	MPI::COMM_WORLD.Gather( &rTime, 1, MPI::DOUBLE, rcvTime, 1, MPI::DOUBLE, 0 );
	avgTime = 0;
	maxTime = 0;
	for( int i = 1; i <= numWorker; i++ ) {
	...
	cout << rank
	<< ": DECODE MA | Avg = " << setw(10) << avgTime/numWorker
	<< "   Max = " << setw(10) << maxTime << endl; 
	...
	cout << rank << ": REDUCE  | Avg = " << setw(10) << avgTime/numWorker << "   Max = " << setw(10) << maxTime << endl;
